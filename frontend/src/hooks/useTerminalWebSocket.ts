...
export const useTerminalWebSocket = ({
  ...
  const getPrompt = useCallback(() => {
    ...
    const shortNamespace = namespace.startsWith('user-')
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (message.type === 'confirm' && message.command) {
          confirmHandlerRef.current?.(message.command)
        }
      } catch (parseError) {
  ...
  }, [WS_BASE_URL, getPrompt, reconnectKey, sessionId, terminal, token])
