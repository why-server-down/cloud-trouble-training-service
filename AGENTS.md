...
#### 7-1. 작업 시작 전 — 선행 조건 판정 (가장 먼저)

코드를 한 줄이라도 쓰기 전에 이 순서를 밟는다.

1. **[docs/parallel-execution-order.md](docs/parallel-execution-order.md)를 읽고**, 지시받은 작업이
   어느 웨이브(Wave 0~4)에 속하는지 작업 ID로 판정한다.
2. **해당 웨이브의 게이트가 충족됐는지 직접 확인한다.** 사람에게 묻지 말고
   실행 순서 문서 §5의 확인 명령을 그대로 돌려서 판단한다
   (dev 머지 여부, `pytest` 결과, `schemas.py` 계약 반영 여부).
3. 계획서에서 그 작업 ID의 **"선행 조건"** 필드도 함께 확인한다.
4. **게이트가 충족됐으면** — 그대로 끝까지 진행한다. 확인을 구하지 않는다.
5. **게이트가 충족되지 않았으면** — 작업을 시작하지 말고 사람에게 아래를 보고한다.

```
[선행 조건 미충족] <작업 ID>는 Wave N 작업입니다.

막힌 이유 : <무엇이 아직 dev에 없는지 — 확인한 명령과 결과>
선행 작업 : <먼저 끝나야 하는 작업 ID>  (담당: 백엔드/프론트/AI)
지금 가능 : <같은 웨이브에서 지금 바로 할 수 있는 작업, 없으면 "없음">
```

- 이 판정이 **에이전트가 멈추는 유일한 지점**이다. 게이트를 통과한 뒤로는
  브랜치 생성부터 머지까지 아무것도 묻지 않는다.
- 게이트 미충족을 무시하고 "일단 만들어두는" 것을 금지한다. 확정되지 않은 계약 위에
  만든 타입·프롬프트·API 호출은 전부 폐기 대상이고, 되돌리는 비용이 훨씬 크다.
- 웨이브 게이트 통과 여부를 **최종 판단하는 것은 사람이다.** 에이전트는 근거(명령과 출력)를
  제시하고, 사람이 "진행"이라고 하면 그때부터 자율로 돈다.
